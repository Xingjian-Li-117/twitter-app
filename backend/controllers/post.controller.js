...
export const createPost = async (req, res) => {
    try {
        const { text } = req.body;
        let { img } = req.body; // may change later with cloudinary, so use let
...
export const likeUnlikePost = async (req, res) => {
    try {
        ...
        if (userLikedPost) { // now unlike post
            await Post.updateOne({ _id: postId }, { $pull: { likes: userId } });
            await User.updateOne({ _id: userId }, { $pull: { likedPosts: postId } });
            // Prevent whole page refresh: remove the user id from the likes array
...
export const getAllPosts = async (req, res) => {
    try {
        ...
        // latest post on top. 
        // populate: get full user information
